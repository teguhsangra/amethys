...
@section('title')
Rakomsis Serviced Office - Editor
@endsection
                ...
                {{ Form::open(array('url' => $form_url, 'method' => $method, 'id' => $form_id, 'enctype' => 'multipart/form-data')) }}
                    ...
                    <div class="row" id="contact_status">
                        ...
                        <div class="col-sm-10 checkbox-radios">
                            ...
                            <div class="form-check">
                                <label class="form-check-label">
                                    <input class="form-check-input" type="radio" name="contact_status" onclick="$('#contact_selector').show();" value="no" @if(!empty($booking)) checked @else checked @endif> No
                                    <span class="circle">
                                        <span class="check"></span>
                                    </span>
                                </label>
                    ...
                    <div class="row" id="price_type_selection" >
                        ...
                        <div class="col-sm-10 checkbox-radios" >
                            <div class="form-check">
                                <label class="form-check-label">
                                    <input class="form-check-input" type="radio" name="price_types" onclick="$('#price_type').val('monthly');resetPeriode();onPeriodeChanged('start_date');" value="monthly"  @if(!empty($inquiry)) @if($inquiry->price_type == 'monthly') checked @endif @endif @if(!empty($booking)) @if($booking->price_type == 'monthly') checked @endif @endif> Monthly
                                    <span class="circle">
                                        <span class="check"></span>
                                    </span>
                                </label>
                            ...
                            <div class="form-check">
                                <label class="form-check-label">
                                    <input class="form-check-input" type="radio" name="price_types" onclick="$('#price_type').val('daily');resetPeriode();onPeriodeChanged('start_date');" value="daily" @if(!empty($inquiry)) @if($inquiry->price_type == 'daily') checked @endif @endif @if(!empty($booking)) @if($booking->price_type == 'daily') checked @endif @endif> Daily
                                    <span class="circle">
                                        <span class="check"></span>
                                    </span>
                                </label>
                                ...
                                <table class="table table-bordered">
                                    <thead class="text-primary text-center">
                                        <tr>
                                            ...
                                            <th width="20%"><div id="complimentary_label">Total Use Complimentary<div id="available_complimentary"></div></div></th>
    ...
    @if(!empty($booking))
        ...
        @foreach($booking->furniture as $no => $furniture)
            ...
            $.get(url_additional_charge, function (data){
                var allow_to_add = true;

                for(var j = 0; j < selected_furniture.length; j++){
                    if(selected_furniture[j].id == '{{ $furniture->id }}'){
                        allow_to_add = false;
                        break;
                    }
                }
    ...
    function setFurniture(){
        ...
        if(selected_furniture.length > 0){
            ...
            for(var i=0; i < selected_furniture.length; i++){
                ...
                furniture += '<td class="text-center" colspan="4"><input type="number" class="form-control text-center" name="fu_quantity[]" id="fu_quantity'+furniture_id+'" min="1" value="'+furniture_quantity+'" oninput="countFurniture()"></td>';
            ...
            if(availability){
                $.get(url, function (data){
                    ...
                    for(var i = 0; i < data.furniture.length; i++){
                        ...
                        for(var j = 0; j < selected_furniture.length; j++){
                            if(data.furniture[i].id == selected_furniture[j].id){
                                allow_to_add = false;
                                break;
                            }
                        }
                        ...
                        if(allow_to_add){
                            item_furniture['id'] = data.furniture[i].id;
                            item_furniture['name'] = data.furniture[i].name;
                            item_furniture['qty'] = data.furniture[i].pivot.quantity;
                            selected_furniture.push(item_furniture);
                        }
                    }
