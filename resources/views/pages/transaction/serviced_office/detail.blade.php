@extends('layouts.app')
@section('title')
Rakomsis Serviced Office - {{ $booking->code }}
@endsection
@section('content')
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header card-header-info card-header-icon">
                ...
                <h4 class="card-title">Detail Serviced Office</h4>
            </div>
            <div class="card-body">
                <a href="{{ url($url) }}" class="btn btn-rose">
                    <i class="fa fa-arrow-left"></i> Back To Serviced Office
                </a>
                <div class="toolbar">
                    ...
                    <div class="btn-group btn-group-md" role="group" >
                        ...
                        <a onclick="print('{{ url($domicile_url) }}')" class="btn  btn-success pull-right text-white">
                            <i class="fa fa-print"></i> Domicile
                        </a>
                        <a onclick="print('{{ url($domicile_url) }}?status=sign')" class="btn  btn-success pull-right text-white">
                            <i class="fa fa-print"></i> Domicile with Sign
                        </a>
                       
                        <a onclick="viewFile('{{ url($term_and_conditions->file_path) }}')" class="btn  btn-warning pull-right text-white">
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <table width="100%" class="table table-bordered" style="vertical-align: center;">
                            ...
                            <tr>
                                ...
                                <td width="25%">
                                    ...
                                    @endif
                                    @if($booking->is_main_agreement == "Y")
                                        : {{$booking->term_notice_period}} Month(s) Notification<br>
                                        :
                                ...
                                <td width="25%" class="text-left" style="top: 50%;">
                                    @if(sizeof($booking->furniture) > 0)
                                        <table class="table" style="background: none !important;">
                                            <thead>
                                                <tr>
                                                    <th colspan="2" style="border:none !important;"><b>Furniture</b></th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                            @foreach($booking->furniture as $furniture)
                                                <tr>
                                                    <td style="border:none !important;">{{ $furniture->name }}</td>
                                                    <td style="border:none !important;">{{ $furniture->pivot->quantity }}</td>
                                                </tr>
                                            @endforeach
                                            </tbody>
                                        </table>
                        ...
                        <table width="100%" class="table table-bordered" style="vertical-align: center;">
                            ...
                            <tr>
                                <td width="20%" style="text-align: center;"><b>Room</b></td>
                            ...
                            @foreach($booking->rooms as $room)
                                <tr>
                                    ...
                                    <td style="padding-top: 10px;padding-bottom: 10px;" class="text-center">
                                        {{ $booking->length_of_term - $booking->free_term_booking }} @if($booking->free_term_booking > 0)(+{{ $booking->free_term_booking }}) @endif
                                        ...
                                        @if($booking->length_of_term_after_office > 0)
                                            <br> {{ $booking->length_of_term - $booking->length_of_term_after_office }} x Rp {{ number_format($room_price, 0, ',', '.') }}
                                            <br> {{ $booking->length_of_term_after_office }} x Rp {{ number_format($after_office_hourly_price, 0, ',', '.') }}
                                        @endif
                                    </td>
                                    <td class="text-center" style="padding-top: 10px;padding-bottom: 10px;">
                                        @if(!empty($booking->term_of_payment)) {{ $booking->term_of_payment }} @else {{ $booking->length_of_term }} @endif
                                    </td>
                                    ...
                                    <td class="text-right" style="padding-top: 10px;padding-bottom: 10px;">
                                        @php
                                            if(empty($booking->term_of_payment)){
                                                $total_term = $booking->length_of_term - $booking->length_of_term_after_office - $booking->free_term_booking;
                                            }else{
                                                $total_term = $booking->term_of_payment;
                                            }
                                            ...
                                            if($booking->length_of_term_after_office > 0) {
                                                if($booking->length_of_term - $booking->length_of_term_after_office > 0){
                                                    $sub_total_main = $sub_total_main + ($room_price * $booking->quantity * $total_term);
                                                ...
                                                if($booking->price_type == 'halfday'){
                                                    ...
                                                    $sub_total_main = $room_price * $booking->quantity * $total_term;
                                                }
                                            }

                                            if($room->has_service_charge == "Y"){
                                                $total_service_charge = $total_service_charge + ($sub_total_main * $service_charge);
                                            }
                            ...
                            @php
                                ...
                                if($booking->tax_status != 'no_tax'){
                                    $total_tax_price = ($total_price + $total_service_charge) * $tax_percentage;
                                }
                            @endphp
                            ...
                            <tr>
                                ...
                                <td class="text-right" style="padding-top: 1px;padding-bottom: 1px;">
                                    Rp {{ number_format($total_service_charge + $booking->total_service_charge_additional_charge, 0, ',', '.') }}
                                </td>
                            </tr>
                            <tr>
                                <td style="padding-top: 1px;padding-bottom: 1px;" colspan="3"></td>
                                ...
                                <td class="text-right" style="padding-top: 1px;padding-bottom: 1px;">
                                    Rp {{ number_format($total_tax_price + $booking->total_tax_additional_charge, 0, ',', '.') }}
                                </td>
                            </tr>
                            <tr>
                                <td style="padding-top: 1px;padding-bottom: 1px;" colspan="3"></td>
                                ...
                                <td class="text-right" style="padding-top: 1px;padding-bottom: 1px;">
                                    <b>Rp {{ number_format($total_price + $total_service_charge + $total_tax_price + $booking->total_additional_charge + $booking->total_service_charge_additional_charge + $booking->total_tax_additional_charge, 0, ',', '.') }}</b>
                                </td>
                            </tr>
                            <tr>
                                <td style="padding-top: 1px;padding-bottom: 1px;" colspan="3"></td>
                            ...
                            <tr>
                                ...
                                <td class="text-right" style="padding-top: 1px;padding-bottom: 1px;"><b>Rp {{ number_format($total_price + $total_service_charge + $total_tax_price + $booking->total_additional_charge + $booking->total_service_charge_additional_charge + $booking->total_tax_additional_charge + $booking->security_deposit + $booking->stamp_duty + $booking->round_price, 0, ',', '.') }}</b></td>
                            </tr>
                        </table>
