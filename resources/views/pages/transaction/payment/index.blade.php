            ...
            <div class="card-body">
                <div class="toolbar">
                    ...
                    <ul class="nav nav-pills nav-pills-warning" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link @if($no == 0) active @endif" data-toggle="tab" href="#payments_{{ $status->id }}" role="tablist">
                                {{ $status->action }}
                            </a>
                        </li>
                        @endforeach
                ...
                <div class="tab-content tab-space">
                    @foreach($statuses as $no => $status)
                    <div class="tab-pane material-datatables table-responsive @if($no == 0) active @endif" id="payments_{{ $status->id }}">
                        <table id="payments_{{ $status->id }}-table" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Code</th>
                                    <th>Customer</th>
...
<script>
    $(function() {
        @foreach($statuses as $no => $status)
	    $('#payments_{{ $status->id }}-table').DataTable({
	        processing: true,
	        serverSide: true,
	        ...
	        columns: [
	            ...
	            { data: 'customer_name', name: 'customer_name' },
	            {
	            	...
	            	"render" : function(row, data, full){
                        var invoice_list = full.invoice_list;
                        var return_html = '';
                        for(var i=0; i < invoice_list.length; i++){
                            return_html += '<p>'+invoice_list[i]+'</p>';
                        }
                        return return_html;
	            	}
                },
	            {
	            	sortable:false,
	            	"render" : function(row, data, full){
                        var deposit_list = full.deposit_list;
                        var return_html = '';
                        for(var i=0; i < deposit_list.length; i++){
                            return_html += '<p>'+deposit_list[i]+'</p>';
                        }
                        return return_html;
	            	}
                },
