...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body">
                <div class="material-datatables" id="meeting_room">
                    {!! $meeting->calendar() !!}
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="tab-content tab-space">
                    ...
                    <div class="tab-pane material-datatables table-responsive @if($no == 0) active @endif" id="booking_{{ $status->id }}">
                        <table id="meeting_rooms_{{ $status->id }}-table" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Location</th>
                                    <th>Code</th>
...
<div class="modal fade model-xl" id="customerContactModel" tabindex="-1" role="dialog" aria-labelledby="customerContactModelLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input type="hidden" name="back_url" value="meeting_room">
...
<script>
    ...
    $(function() {
        @foreach($statuses as $no => $status)
	    $('#meeting_rooms_{{ $status->id }}-table').DataTable({
	        processing: true,
	        serverSide: true,
	        ...
	        columns: [
	            ...
	            { data: 'status_name', name: 'status_name' },
	            {
	            	...
	            	"render" : function(row, data, full){
                        ...
                        return_html += ' <a rel="tooltip" class="btn btn-round btn-default" title="Edit Contact" href="'+booking_contact_url+'?back_url=meeting_room&menu_name=Meeting Room&booking_id='+id+'"><i class="material-icons">assignment_ind</i></a>';
    ...
    $("#filter").on('click', function(){
        ...
        window.location.href = '{{ url('meeting_room')}}'+"?location_id="+location_id;
    });
                ...
                date_list += '<table  class="table table-striped table-bordered table-hover table-responsive" cellspacing="0" width="100%" style="width:100%" id="schedule_bookings">';
                    ...
                    date_list += '<tbody >';
                        for(var i=0; i < data.rooms.length; i++){
                            date_list += '<tr>';
                                date_list += '<td class="table-info">'+data.rooms[i]['room_number']+'</td>';

                                for(var j=0; j < data.days.length; j++){
                                    var detail = data.list[j][i];
                                    var detail_url = "{{ url('meeting_room') }}";
                                    var detail_style = '';
