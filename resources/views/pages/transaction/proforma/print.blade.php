...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-8">
            ...
            <p>{!! $proforma->customer->address !!}</p>
            <br>
            @if($proforma->reason != null)
            <h5><b>Reason</b></h5>
            <p>{{ $proforma->reason }}</p>
            @endif
        </div>
        <div class="col-md-4 text-right">
            ...
            <table style="float: right;" width="100%">
                ...
                <tr>
                    ...
                    <td class="text-right">{{$proforma->code}}</td>
                </tr>
                @if($proforma->has_po == 'Y')
                <tr>
                    <td class="text-left">PO No #:</td>
                    <td class="text-right">{{$proforma->po_number}}</td>
                </tr>
                @endif
                <tr>
                    <td class="text-left">Date : </td>
                    <td class="text-right">{{date("j F Y",strtotime($proforma->proforma_date))}}</td>
                </tr>
                <tr>
                    <td class="text-left">Due Date : </td>
                    <td class="text-right">{{date("j F Y",strtotime($proforma->due_date))}}</td>
                </tr>
            </table>
        </div>
    </div>
            ...
            <table class="table table-border">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            IDR {{number_format($detail_price) }} <br>
                        </td>
                        <td class="text-right">
                            IDR {{number_format($proforma_detail->detail_price)}} <br>
                        </td>
                    </tr>
                    @else
                    @php
                    ...
                    for($i=0; $i < sizeof($detail_proforma); $i++){
                        ...
                        $new_array['remarks'] = $proforma_detail->remarks;
                        ...
                        @for($i=0; $i < sizeof($detail_proforma); $i++)
                            ...
                            <td class="text-right">
                                IDR {{number_format($detail_proforma[$i]['quantity'] * ($detail_proforma[$i]['detail_price']))}} <br>
                            </td>
                            </tr>
                            @endfor
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                            	    {{ date('j M Y', strtotime($proforma->inquiry->start_date)) }} - {{ date('t M Y', strtotime($proforma->inquiry->start_date. '+' . ($proforma->inquiry->term_of_payment - 1) . ' months')) }}
				</td>
                                <td class="text-center">
                                    @php
                                    $length_of_term = $proforma->inquiry->length_of_term;
			    ...
			    @if($proforma->total_price_on_tax > 0)
                            <tr>
                                ...
                                <td class="text-right"><b>IDR
                                        {{number_format($proforma->total_price_on_tax)}}</b></td>
                            </tr>
			    @endif
                            <tr>
